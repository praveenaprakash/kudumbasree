...
$s=mysqli_query($con,"select * from unitreg");
 echo "<table border ='1' frame=border cellpadding=8 bgcolor=#456667 bordercolor=red cellspacing=5 width=90% height=80% align='center' style=margin:80px;>
		<tr><th colspan=12>  NHG UNITS</font></th></tr>
		<tr><th>Uname</th><th>sdate</th><th>wardno</th><th>nomember</th><th>president</th><th>Secretary</th><th>bname</th><th>Acno</th></tr>";
	   while($row=mysqli_fetch_array($s))
	   {
		   $vid=$row[0];
	   echo"<tr><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td><td>$row[5]</td><td>$row[6]</td><td>$row[7]</td><td>$row[8]</td><td><form method='post' action='View Unit.php'><input type='hidden' name='h1' value='$vid'><input type='submit' name='edit' value='edit'></form></td></tr>";	  
	    }
	   echo"</table>"
	   ?>
	</body>
