...
$s=mysqli_query($con,"select * from loan");
 echo "<table border ='1' frame=border cellpadding=8  bordercolor=red cellspacing=4 width=80% height=70% align='center' style=margin:80px;>
	   ...
	   {
		   $loanid=$row[0];
	   echo"<tr><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td><td>$row[5]</td><td>$row[6]</td><td>$row[7]</td><td>$row[8]</td><td>$row[9]</td><td><input type='hidden' name='h1' value='$loanid'><input type='submit' name='Apply' value='Apply' ></form></td></tr>";	  
	    }
	   echo"</table>"
	   ?>
	</body>
