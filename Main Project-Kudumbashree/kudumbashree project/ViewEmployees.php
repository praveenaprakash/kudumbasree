...
$s=mysqli_query($con,"select * from empreg");
 echo "<table border ='1' frame=border cellpadding=8 bgcolor=#456667  cellspacing=5 width=90% height=80% align='center' style=margin:80px;>
		...
		<tr><th>NAME</th><th>ADDRESS</th><th>DOB</th><th>PHONE</th><th>USERTYPE</th><th>OCCUPATION</th><th>UNIT NAME</th><th>Action</th></tr>";
	   while($row=mysqli_fetch_array($s))
	   {
		   $rid=$row[0];
	   echo"<tr><td>$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td><td>$row[5]</td><td>$row[6]</td><td>$row[7]</td><td><td><form method='post' action='ViewEmployee.php'><input type='hidden' name='h1' value='$rid'><input type='submit' name='Edit' value='Edit'></form></td></tr>";	  
	    }
	   echo"</table>"
	   ?>
	</body>
